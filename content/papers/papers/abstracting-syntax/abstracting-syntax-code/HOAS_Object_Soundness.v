...
Require Export HOAS_Object_Infrastructure.
...
Proof with auto.
  ...
  Case "wf_typ_var".
    ...
    simpl_env in *...
  Case "wf_typ_all".
    apply notin_union. eapply_first_hyp; eauto.
    pick fresh Y.
...
Lemma expr_from_wf_exp : forall E e,
  wf_exp E e -> expr e.
Proof.
  intros E e H; induction H; try econstructor; eauto using type_from_wf_typ.
Qed.
...
Proof with simpl_env; auto.
  ...
  Case "sub_all / sub_top".
    assert (sub E0 (typ_all S1 (abs S2)) (typ_all T1 (abs T2))).
...
Proof with auto.
  ...
  remember (exp_abs S1 (abs e1)) as e.
...
Lemma typing_inv_tabs : forall E S1 e1 T,
  typing E (exp_tabs S1 (abs e1)) T ->
  forall U1 U2, sub E T (typ_all U1 (abs U2)) ->
     sub E U1 S1
  /\ exists S2, exists L, forall X, X `notin` L ->
...
Proof with simpl_env; auto.
  intros E S1 e1 T Typ.
  remember (exp_tabs S1 (abs e1)) as e.
  generalize dependent e1.
  generalize dependent S1.
...
Lemma canonical_form_abs : forall e U1 U2,
  ...
  exists V, exists e1, e = exp_abs V (abs e1).
Proof.
...
Lemma canonical_form_tabs : forall e U1 U2,
  value e ->
  typing empty e (typ_all U1 (abs U2)) ->
  exists V, exists e1, e = exp_tabs V (abs e1).
Proof.
  intros e U1 U2 Val Typ.
  remember empty as E.
  remember (typ_all U1 (abs U2)) as T.
  ...
  Case "typing_case".
    ...
    SCase "Val1".
      ...
      SSCase "Left J1".
        ...
        assert (expr (exp_case (exp_inl e4) (abs e2) (abs e3))) by auto.
        inversion H3...
      SSCase "Right J1".
        subst.
        exists (open_ee e3 e4)...
        inversion Val1; subst.
        assert (expr (exp_case (exp_inr e4) (abs e2) (abs e3))) by auto.
        inversion H3...
Qed.
