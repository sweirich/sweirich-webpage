...
Lemma app_head_eq_nil : forall (A : Type) (xs ys : list A),
  ...
  xs = nil.
Proof.
  induction xs; intros ys H; auto.
  assert (J : length ys = length ((a :: xs) ++ ys)) by (f_equal; auto).
  simpl in J.
  autorewrite with list in J.
...
Lemma app_inj_head : forall (A : Type) (xs xs' ys : list A),
  ...
  xs = xs'.
Proof.
  induction xs as [ | x xs IH ]; intros xs' ys Eq; simpl in *.
  symmetry; eauto using app_head_eq_nil.
  induction xs'; simpl in *.
    assert (H : length (x :: xs ++ ys) = length ys) by (f_equal; auto).
      simpl in H.
