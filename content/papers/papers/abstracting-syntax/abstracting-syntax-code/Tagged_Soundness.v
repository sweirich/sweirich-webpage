...
Proof with simpl_env; eauto.
  ...
  inversion WA; subst.
  pick fresh X.
  rewrite (subst_tt_intro Typ Typ X)...
  rewrite_env (nil ++ E).
...
Proof with auto.
  ...
  Case "wf_typ_all".
    ...
    pick fresh Y.
    apply (notin_fv_tt_open Typ Typ Y).
...
Proof with auto.
  intros x u V.
  remember Typ as t.
  induction V; try discriminate; subst; simpl; f_equal...
  Case "fvar".
    destruct (a == x)...
Qed.
...
Proof with simpl_env; auto.
  intros L E e v H J.
  pick fresh x.
  rewrite (subst_ee_intro Exp Exp x)...
  rewrite_env (nil ++ E).
...
Proof with simpl_env; auto.
  intros L E e V H J.
  pick fresh X.
  rewrite (subst_te_intro Exp Typ X)...
  rewrite_env (nil ++ E).
...
Proof with simpl_env; auto.
  ...
  Case "sub_all / sub_top".
    assert (sub E0 (typ_all S1 S2) (typ_all T1 T2)).
  ...
  Case "sub_trans_tvar".
    ...
    SCase "X = Z".
      ...
      SSCase "left branch".
        ...
        apply sub_weakening...
      SSCase "right branch".
        rewrite (subst_tt_fresh Typ Typ Z P Q).
          binds_get H.
            inversion H1; subst...
