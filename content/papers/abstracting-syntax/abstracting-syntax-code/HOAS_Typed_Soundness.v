...
Lemma wf_senv_from_wf_exp : forall E e,
  ...
  wf_senv E.
Proof.
  induction 1; eauto using wf_senv_from_wf_typ.
Qed.
...
Lemma wf_senv_from_wf_env : forall E,
  ...
  wf_senv (to_senv E).
Proof.
  induction 1; simpl_env in *; eauto.
Qed.
...
Proof with simpl_env; eauto.
  induction F; intros x b J; simpl_env in *.
  inversion J...
  inversion J; subst; simpl_env in *.
    inversion J...
    inversion J...
Qed.
...
Lemma ok_from_wf_typ : forall E T,
  wf_typ E T -> ok E.
Proof.
  intros E T H; induction H; eauto using ok_from_wf_senv.
Qed.
...
Proof with simpl_env in *; eauto.
  ...
  Case "wf_typ_var".
    ...
    assert (X `notin` dom (G ++ F)).
      assert (Hok' : (ok (G ++ F ++ E))) by auto using ok_from_wf_senv.
      rewrite_env ((G ++ F) ++ E ++ nil) in Hok'.
      eapply fresh_mid_head_In; eauto.
    idtac...
  Case "wf_typ_all".
...
Lemma wf_typ_strengthening : forall E F x T,
 wf_typ (F ++ [(x, Exp)] ++ E) T ->
 wf_typ (F ++ E) T.
Proof with simpl_env; eauto using wf_senv_strengthening.
  intros E F x T H.
...
Proof with simpl_env; eauto using wf_typ_weaken_head,
                                  wf_senv_strengthening,
                                  type_from_wf_typ.
...
Lemma wf_typ_from_binds_sub : forall x U E,
  ...
  wf_typ (to_senv E) U.
Proof with eauto using wf_senv_from_wf_typ, wf_typ_weaken_head.
...
Proof with auto.
  ...
  induction Wf_typ; intros F Eq Fr; subst; simpl;
    try solve [repeat apply notin_union; auto; eapply_first_hyp; eauto]...
  Case "wf_typ_var".
...
Proof with auto.
  ...
  Case "wf_typ_var".
    assert (x <> X)...
    intro; subst.
    binds_get H0; auto using ok_from_wf_senv.
  Case "wf_typ_all".
...
Proof with simpl_env; intuition (eauto using wf_senv_from_wf_typ).
  intros E S T H.
  induction H...
  Case "sub_all".
    repeat split...
...
Proof with simpl_env; try solve [intuition].
  ...
  Case "typing_var".
    repeat split...
    eauto using wf_senv_from_wf_env.
...
Lemma sub_weakening : forall E F G S T,
  ...
  wf_env (G ++ F ++ E) ->
  sub (G ++ F ++ E) S T.
Proof with simpl_env in *; auto using wf_senv_from_wf_env, wf_typ_weakening.
  ...
  Case "sub_top".
    ...
    repeat rewrite <- map_concat; auto using wf_senv_from_wf_env.
  Case "sub_refl_tvar".
    constructor...
    apply wf_typ_weakening...
    repeat rewrite <- map_concat; auto using wf_senv_from_wf_env.
...
Proof with
      ...
      eauto using wf_typ_subst_tb, wf_env_subst_tb, wf_typ_weaken_head,
                  wf_senv_from_wf_typ, wf_senv_strengthening,
